import json
f = open('C:\Mock_Hackathon\Mock-Hackathon\Level1\level1a.json')
data = json.load(f)
f.close()
distances=[]
...
while max(tempQuantity)>0:
    ...
    for i in tempQuantity:
        while max(tempQuantity)>0 and tempCapacity-max(tempQuantity)>=0:
            index.append(tempQuantity.index(max(tempQuantity)))
...
for i in range(0,len(paths)):
    ...
    count+=1
print(output)
with open('C:\Mock_Hackathon\Mock-Hackathon\Level1\level1a_output.json', 'w') as f:
    json.dump(output, f)